...
class ItemView : FrameLayout {
    ...
    private val hideOTPDrawable: Int = 0
    ...
    private fun generateViews(styles: TypedArray) {
        val defaultHeight = Utils.getPixels(mContext!!, DEFAULT_BAR_HEIGHT)
        val defaultOtpTextSize = Utils.getPixels(mContext!!, DEFAULT_OTP_TEXT_SIZE)
        val textColor = styles.getColor(
            R.styleable.OtpTextView_android_textColor,
            ResourcesCompat.getColor(mContext!!.resources, R.color.colorPagerDesc, null)
        )
        val barHeight = styles.getDimension(R.styleable.OtpTextView_otp_bar_height,
            defaultHeight.toFloat()
        )
        val barMargin =
            styles.getDimension(R.styleable.OtpTextView_bar_margin,
                Utils.getPixels(mContext!!, 0).toFloat()
            )
        ...
        defaultOTPDrawable =
            ResourcesCompat.getColor(mContext!!.resources, android.R.color.transparent, null)

        val barEnabled = styles.getBoolean(R.styleable.OtpTextView_bar_enabled, false)

        val otpTextSize =
            styles.getDimension(R.styleable.OtpTextView_otp_text_size, defaultOtpTextSize.toFloat())
...
//        this.setBackgroundResource()
        ...
        if (otpTextTypeFace != null) {
            try {
                val tf = Typeface.createFromAsset(mContext!!.assets, otpTextTypeFace)
        ...
        val typeface = ResourcesCompat.getFont(mContext!!, R.font.medium)
        ...
        if (barEnabled) {
            val barViewParams =
                LayoutParams(LayoutParams.MATCH_PARENT, barHeight.toInt())
