...
class LoadingView(context: Context, atr: AttributeSet?) : View(context, atr) {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        internalRadius = typeArray.getDimension(
            R.styleable.LoadingView_lv_internal_radius,
            DEFAULT_INTERNAL_RADIUS.toFloat()
        )
        externalRadius = typeArray.getDimension(
            R.styleable.LoadingView_lv_external_radius,
            DEFAULT_EXTERNAL_RADIUS.toFloat()
        )
        ...
        if (colorList.size == 0) {
            colors = intArrayOf(
                ContextCompat.getColor(context, R.color.color_start),
                ContextCompat.getColor(context, R.color.color_end)
            )
        } else {
    ...
    private fun setShader() {
        val mLinearGradient = LinearGradient(
            mWidth / 2 - externalRadius,
            mHeight / 2 - externalRadius,
            mWidth / 2 - externalRadius,
            mHeight / 2 + externalRadius,
            colors!!,
            null,
    ...
    fun start() {
        if (disposable == null || disposable!!.isDisposed) {
            //if(mDuration < 30)
            // mDuration = 30
            disposable = Observable.interval(mDuration.toLong(), TimeUnit.MILLISECONDS)
    ...
    private fun update() {
        ...
        angle1 = angle1 + 10    //previously 10     //changed speed
        if (angle1 >= 360) {
            angle1 = 0
            cyclic++
        }
                ...
                if (i == index) {
                    if (angle1 % radian == 0) {
                        ...
                        canvas.drawCircle(
                            getCircleX(angle1),
                            getCircleY(angle1),
                            if (smallerCircleRadius < internalRadius) internalRadius else smallerCircleRadius,
                            mPaint!!
                        )
                    }
                } else if (i == index + 1) {
                    if (angle1 % radian == 0) {
                        ...
                        canvas.drawCircle(
                            points!![i].x,
                            points!![i].y,
                            if (biggerCircleRadius < internalRadius) internalRadius else biggerCircleRadius,
                            mPaint!!
                        )
                    }
                } else if (i > index + 1) {
    ...
    private fun drawBezier(canvas: Canvas) {
        ...
        if (isEvenCyclic()) {
            if (angle1 % radian < radian / 2) {
                ...
                mPath.quadTo(
                    rightX + (leftX - rightX) / (radian / 2) * if (angle1 % radian > radian / 2) radian / 2 else angle1 % radian,
                    rightY + (leftY - rightY) / (radian / 2) * if (angle1 % radian > radian / 2) radian / 2 else angle1 % radian,
                    pointF2.x,
                    pointF2.y
                )
                ...
                mPath.quadTo(
                    leftX + (rightX - leftX) / (radian / 2) * if (angle1 % radian > radian / 2) radian / 2 else angle1 % radian,
                    leftY + (rightY - leftY) / (radian / 2) * if (angle1 % radian > radian / 2) radian / 2 else angle1 % radian,
                    pointF1.x,
                    pointF1.y
                )
            ...
            if (circleIndex > 0 && angle1 % radian > radian / 2) {
                ...
                mPath.moveTo(pointF3.x, pointF3.y)
                mPath.quadTo(
                    rightX + (leftX - rightX) / (radian / 2) * if (radian - angle1 % radian > radian / 2) radian / 2 else radian - angle1 % radian,
                    rightY + (leftY - rightY) / (radian / 2) * if (radian - angle1 % radian > radian / 2) radian / 2 else radian - angle1 % radian,
                    pointF2.x,
                    pointF2.y
                )
                mPath.lineTo(pointF3.x, pointF3.y)
                ...
                mPath.moveTo(pointF4.x, pointF4.y)
                mPath.quadTo(
                    leftX + (rightX - leftX) / (radian / 2) * if (radian - angle1 % radian > radian / 2) radian / 2 else radian - angle1 % radian,
                    leftY + (rightY - leftY) / (radian / 2) * if (radian - angle1 % radian > radian / 2) radian / 2 else radian - angle1 % radian,
                    pointF1.x,
                    pointF1.y
                )
                mPath.lineTo(pointF4.x, pointF4.y)
...
//        animators!!.clear()
//
//        val circleGetSmallerAnimator =
//            ValueAnimator.ofFloat(getMaxInternalRadius(), getMinInternalRadius())
//        circleGetSmallerAnimator.duration = 5000L
//        circleGetSmallerAnimator.addUpdateListener { animation ->
//            smallerCircleRadius = animation.animatedValue as Float
//        }
//        animators!!.add(circleGetSmallerAnimator)
//
//        val circleGetBiggerAnimator =
//            ValueAnimator.ofFloat(getMinInternalRadius(), getMaxInternalRadius())
//        circleGetBiggerAnimator.duration = 5000L
//        circleGetBiggerAnimator.addUpdateListener { animation ->
//            biggerCircleRadius = animation.animatedValue as Float
//        }
